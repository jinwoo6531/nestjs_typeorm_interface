...
export class ProductsRepository extends Repository<ProductEntity> {
  ...
  private init(user: UserEntity): SelectQueryBuilder<ProductEntity> {
    ...
    return this.createQueryBuilder('a')
      ...
      .innerJoinAndMapMany('a.categories', 'category', 'c', 'b.category_id = c.id')
      .innerJoinAndMapMany('a.thumbnails', 'thumbnail', 'd', 'a.id = d.product_id')
      ...
      .leftJoinAndMapOne('a.favorite', 'favorite', 'x', 'a.id = x.product_id and x.user_id = :user_id', { user_id })
  }
  ...
  async filter(dto: GetProductsFilterDao): Promise<MetaData> {
    ...
    if (category_id) {
      query.innerJoin('product_category', 'aa', 'a.id = aa.product_id')
        .andWhere('aa.category_id = :category_id', { category_id });
    }
    if (q) {
      query.leftJoin('product_tag', 'bb', 'a.id = bb.product_id')
